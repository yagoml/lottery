module.exports = (grunt) ->
	...
	renameToCompiled = (dest, src, config) ->
		dest + src.substring(0,src.lastIndexOf(".")) + config.ext
	...
	grunt.event.on 'watch', (action, filepath, target) ->
		configs = grunt.util._.extend({}, appFiles[target])
		...
		if (action is 'deleted')
			...
			switch (target)
				when "coffee" then grunt.config('coffee.compile.files', [configs])
				when "less" then grunt.config('less.dev.files', [configs])
				else break

	grunt.registerTask 'env', 'Configura arquivos para ambiente de acordo', (environment) ->
		envConfig = app = version = indexHtml = path = require("path")
		grunt.log.ok("Iniciando configuração de ambiente... ")

		# Obtém o diretório do Projeto
		# obtendo assim o nome do projeto atual
	...
	grunt.registerTask('dev',['coffee', 'less:dev', 'cssmin', 'uglify', 'copy', 'env:development'])
	grunt.registerTask('development', ['coffee', 'less:dev', 'cssmin', 'uglify', 'copy', 'env:development'])
	grunt.registerTask('test',['coffee', 'less:dev', 'cssmin', 'copy', 'env:test'])
	grunt.registerTask('production', ['coffee', 'less', 'cssmin', 'uglify', 'copy', 'env:production'])
	grunt.registerTask('default', ['coffee', 'less', 'cssmin', 'uglify', 'copy', 'env:production'])