<div class="container-fluid margB20">
    <form id="extratoForm" class="datesFilter" method="get" action="<?php base_url('usuarios/comissoes') ?>">
...
<?php if (isset($comissoes)) { ?>
    <?php if ($comissoes) { ?>
        <span class="pad5 alert-success pull-right">Total: <b>R$ <?php echo number_format($total, 2, ',', '.'); ?></b></span>
        <table class="table table-hover extratosTable">
            ...
            <?php foreach ($comissoes as $key => $comissao) { ?>
                <tr>
                    <td><?php echo date('d/m/Y à\s H:i', strtotime($comissao->data)) ?></td>
                    <td><?php echo $comissao->nomeOp ?></td>
                    <td><?php echo $comissao->apelido ?></td>
                    <td class="text text-success"><?php echo '+ ' . number_format($comissao->valor, 2, ',', '.') ?></td>
                </tr>
            <?php } ?>
        </table>
    ...
    <?php } else { ?>
        <div class="responseMsg alert alert-info text-center">
            ...
            Nenhuma comissão creditada nesse período.
        </div>
    <?php } ?>
<?php } ?>
