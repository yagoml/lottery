...
function compareGanhos($a, $b) {
    return $a->ganhos < $b->ganhos;
}
...
function generateModels($db) {
    ...
    foreach ($tables as $table) {
        $tableName = ucfirst($table->$indexTables);
        $fields = $ci->db->field_data($tableName);
        $model = fopen('application/models/' . $namespace . '/' . $tableName . '.php', 'w');
        $modelInfo = '';
        ...
        foreach ($fields as $field) {
            if (!$field->primary_key) {
                $modelInfo .= '        $this->' . $field->name . ' = ' . initField($field->type) . ';' . PHP_EOL;
            }
        }
        ...
        fwrite($model, '<?php ' . PHP_EOL . PHP_EOL
                ...
                . 'class ' . $tableName . '{' . PHP_EOL . PHP_EOL
                . $modelInfo
                . '}'
        );
        fclose($model);
...
/**
 * Monta a tabela de Níveis
 * @author Yago M. Laignier <yagoskor@gmail.com>
 */
function niveisTable() {
    $ci = & get_instance();
    $exp = 50;
    $next = 60;
    $niveis = array();

    if ($ci->db->query("SELECT * FROM niveis")) {
        $ci->db->query("TRUNCATE TABLE niveis");
    } else {
        $ci->db->query("CREATE TABLE niveis(`nivel` INT NOT NULL, `exp` INT NULL, PRIMARY KEY (`nivel`))");
    }

    $ci->db->query("INSERT INTO niveis SET nivel=1, exp=0");
    for ($x = 2; $x <= 100; $x++) {
        $niveis[$x] = ceil($exp);
        $ci->db->query("INSERT INTO niveis SET nivel = $x, exp = " . $niveis[$x]);
        echo "Nível: $x, Experiência: " . $niveis[$x] . "<br>";
